...
- Stabilize transient scroll anchoring or lazy-layout displacement before each bitmap while rejecting persistently redirected positions.
- Accept Firefox scroll offsets that differ only by half a physical pixel plus a minimal floating-point epsilon, while continuing to reject material displacement.
...
All listed MVP functions are implemented. Automated coverage includes document scrolling, nested-container scrolling, transient scroll-position recovery, Firefox subpixel quantization, frame-aware stitching, adaptive large-page output, cleanup, diagnostics, and failure paths. The corrected real-page path still requires a final Firefox retest on the reported page.
